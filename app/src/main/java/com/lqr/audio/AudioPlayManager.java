...
public class AudioPlayManager implements SensorEventListener {
            ...
            if (this._mediaPlayer.isPlaying()) {
                if ((double) range > 0.0D) {
                    ...
                    try {
                        ...
                        this._mediaPlayer.setDataSource(this.context, this._playingUri, tinodeFileHeader());
    ...
    private void setScreenOff() {
        if (this._wakeLock == null) {
            if (Build.VERSION.SDK_INT >= 21) {
                this._wakeLock = this._powerManager.newWakeLock(32, "tinode:AudioPlayManager");
            } else {
    ...
    private Map<String, String> tinodeFileHeader() {
        Map<String, String> headers = new HashMap<>();
        headers.put("X-Tinode-APIKey", Cache.getTinode().getApiKey());
        headers.put("Authorization", "Token " + Cache.getTinode().getAuthToken());
        return headers;
    }
    ...
    public void startPlay(Context context, Uri audioUri, IAudioPlayListener playListener) {
        if (context != null && audioUri != null) {
            ...
            try {
                ...
                this._mediaPlayer.setDataSource(context, audioUri, tinodeFileHeader());
                this._mediaPlayer.setAudioStreamType(3);
