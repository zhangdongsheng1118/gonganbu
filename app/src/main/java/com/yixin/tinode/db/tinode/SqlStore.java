...
class SqlStore implements Storage {
    ...
    public boolean topicUpdateState(Topic topic) {
        return TopicDb.updateState(mDbh.getWritableDatabase(), topic);
    }
    ...
    public long msgReceived(Topic topic, Subscription sub, MsgServerData m) {
        ...
        //set before insert into topic db
        if(m.seq >= topic.getSeq()) {
            topic.setLastAct(m.from);
            topic.setLastMsg(SessionAdapter.getMsgText(msg, ""));
            topic.setRecv(m.seq);
            topic.setRead(m.seq);
            topic.setTouched(m.ts);
            if (sub.pub != null) {
                topic.setUserName(((VCard) sub.pub).fn);
            } else {
    ...
    @Override
    public boolean msgUpdate(long id, Drafty data) {

        SQLiteDatabase db = mDbh.getWritableDatabase();
        StoredMessage msg = MessageDb.readOne(db, id);
        ...
        return MessageDb.update(db, msg);
    }

    //发送消息到本地，但是不发送到服务器
    @Override
    public long msgSendLocal(Topic topic, Drafty data) {
    ...
    private long insertMessage(Topic topic, Drafty data, int initialStatus) {
        ...
        // Set seq to zero. MessageDb will assign a unique temporary (nagative int) seq.
        // The temp seq will be updated later, when the message is received by the server.
        //pcg change from 0 to -1,because we add a new way to set to negative in insert
        msg.seq = 0;
    ...
    @Override
    public boolean msgFailed(Topic topic, long messageDbId, Drafty data) {
        return MessageDb.updateStatusAndContent(mDbh.getWritableDatabase(), messageDbId, BaseDb.STATUS_ERROR, data);
    }
