...
public class TopicDb implements BaseColumns {
     ...
     * Private topic description, blob
     */
    public static final String COLUMN_NAME_NAME_SEARCH = "sear";
    /**
     * 存在private中，
     */
    ...
    //added by pcg
    public static final String COLUMN_NAME_LASTACT="lastact";

    public static final String COLUMN_NAME_LASTMSG="lastmsg";
    ...
    static final int COLUMN_IDX_NAME_SEARCH = 21;
    static final int COLUMN_IDX_TOP = 22;
    //是否已删除聊天
    static final int COLUMN_IDX_STATE = 24;
    //added by pcg
    static final int COLUMN_IDX_LASTACT=25;
    static final int COLUMN_IDX_LASTMSG=26;

    static final int COLUMN_IDX_NEXT_UNSENT_SEQ = 23;

    static final int LAST_COLUMN_IDX = COLUMN_IDX_LASTMSG;

    //不存在于表中，联查时使用
    static final int COLUMN_IDX_SEARCH = LAST_COLUMN_IDX + 1;
    static final int COLUMN_IDX_TS = LAST_COLUMN_IDX + 2;
    static final int COLUMN_IDX_PUB = LAST_COLUMN_IDX + 3;
    ...
    static final String CREATE_TABLE =
            "CREATE TABLE " + TABLE_NAME + " (" +
                    ...
                    COLUMN_NAME_NAME_SEARCH + " TEXT," +
                    COLUMN_NAME_TOP + " INT," +
                    COLUMN_NAME_NEXT_UNSENT_SEQ + " INT," +
                    COLUMN_NAME_STATE + " INT DEFAULT 0," +
                    COLUMN_NAME_LASTACT + " TEXT," +
                    COLUMN_NAME_LASTMSG+ " TEXT)";
    /**
    ...
    public static long insert(SQLiteDatabase db, Topic topic) {
        ...
        values.put(COLUMN_NAME_TOP, topic.getIsTop());
        ...
        if (topic.getPub() instanceof VCard) {
            values.put(COLUMN_NAME_NAME_SEARCH, ((VCard) topic.getPub()).fn);
        }

        values.put(COLUMN_NAME_STATE, Topic.STATE_NORMAL);
        values.put(COLUMN_NAME_LASTACT, topic.getLastAct());
        values.put(COLUMN_NAME_LASTMSG, topic.getLastMsg());
    ...
    public static boolean updateTop(SQLiteDatabase db, Topic topic, int top) {
        return false;
    }
    ...
    public static void updateLastUsed(SQLiteDatabase db, String topic) {
        db.execSQL("update topics set last_used=" + new Date().getTime() + " where name='" + topic + "'");
    }
...
//                " ORDER BY " + COLUMN_NAME_LASTUSED + " DESC";
        ...
        String sql = "select topic.*,msg.search,msg.ts,users.pub from \n" +
                "(select * from " + TABLE_NAME + " where " + COLUMN_NAME_ACCOUNT_ID + "=" + accountId +
                " ) topic" +
                " left join \n" +
                "(select search,content,ts,user_id,topic_id from " + MessageDb.TABLE_NAME + " where status<=" + BaseDb.STATUS_VISIBLE + "   group by topic_id having max(ts)) msg\n" +
                " on topic._id=msg.topic_id \n" +
                " left join \n" +
                " (select _id,pub from " + UserDb.TABLE_NAME + " where account_id=" + accountId + ") users  \n" +
                " on msg.user_id=users._id \n" +
                " order by topic.top desc,msg.ts desc,topic." + COLUMN_NAME_LASTUSED + " DESC";
        ;
     ...
     * @return Subscription
     */
    public static Topic readOne(SQLiteDatabase db, Tinode tinode, String name) {
        Topic topic = null;
        String sql = "SELECT * FROM " + TABLE_NAME +
                " WHERE " +
    ...
    //删除聊天和恢复聊天
    public static boolean updateState(SQLiteDatabase db, long topicId, int state) {
        return BaseDb.update(db, TABLE_NAME, COLUMN_NAME_STATE, topicId, state);
    }

    public static boolean updateState(SQLiteDatabase db, Topic topic) {
        StoredTopic st = (StoredTopic) topic.getLocal();
        if (st != null) {
            BaseDb.update(db, TABLE_NAME, COLUMN_NAME_TOP, st.id, topic.getIsTop());
            BaseDb.update(db, TABLE_NAME, COLUMN_NAME_RECV, st.id, topic.getRecv());
            BaseDb.update(db, TABLE_NAME, COLUMN_NAME_READ, st.id, topic.getRead());
            return BaseDb.update(db, TABLE_NAME, COLUMN_NAME_STATE, st.id, topic.getState());
        }
