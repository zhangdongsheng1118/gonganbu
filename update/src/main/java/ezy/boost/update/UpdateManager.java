...
//    public static final String Url = BaseUrl + "/android/app!check.do?ver.pack=${pack}&ver.version=${version}&ver.channel=app";

    public static final String BaseUrl = "http://123.56.100.207:8090";
//            public static final String BaseUrl = "http://221.218.252.146:8080";
    public static final String Url = BaseUrl + "/app/check?ver.pack=${pack}&ver.version=${version}&ver.channel=app";

    public static void checkVersion(Context context) {
        //TODO
//        UpdateManager.create(context).setUrl(Url.replace("${pack}", AppUtils.getAppPackageName()).replace("${version}", AppUtils.getAppVersionCode() + "")).setOnFailureListener(
...
//        ).check();
    ...
    public static void checkVersionManu(Context context) {
        LogUtils.e("手动更新");
        UpdateManager.create(context).setUrl(Url.replace("${pack}", AppUtils.getAppPackageName()).replace("${version}", AppUtils.getAppVersionCode() + "")).setManual(true).setOnFailureListener(
                new OnFailureListener() {
                    @Override
                    public void onFailure(UpdateError error) {
