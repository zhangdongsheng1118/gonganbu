...
public class Drafty implements Serializable {
    ...
    @JsonIgnore
    public String getImageUrl(boolean isThumbnail, URL baseUrl) {
        String image = "";
        try {
            Drafty.Style[] fmt = getStyles();
            if (fmt != null) {
                Drafty.Style style = fmt[0];
                Map<String, Object> data = getEntity(style).getData();

                image = (String) data.get("val");
                if (image != null) {
                    Integer type = (Integer) data.get("type");
                    if (type == null || type == Drafty.SEND_FILE_TYPE_TINODE) {
                        image = new URL(baseUrl, image).toString();
                    } else {
                        if (isThumbnail) {
                            image = image.startsWith("http") ? image + "?width=200" : "http://" + image + "?width=200";
                        } else {
    ...
    @JsonIgnore
    public String getVoiceUrl(URL baseUrl) {
        String image = null;
        try {
            Drafty.Style[] fmt = getStyles();
            if (fmt != null) {
    ...
    public Drafty(String text, Style[] fmt, Entity[] ent) {
        this.txt = text;
        this.fmt = fmt;
        this.ent = ent;
    }
    ...
    public String[] getEntReferences() {
        ...
        for (Entity anEnt : ent) {
            if (anEnt != null && anEnt.data != null) {
                ...
                //兼容视频消息，一条视频消息有两条文件信息：封面和视频
//                Object coverRef = anEnt.data.get("cover");
//                if (coverRef != null) {
//                    result.add((String) coverRef);
//                }
            }
        }
    ...
    public static class CollectDes {
        private String txt;
    ...
    //ADD
    public static final int MEMBER_OPERATE_TYPE_ADD = 0;
    //REMOVE
    public static final int MEMBER_OPERATE_TYPE_REMOVE = 1;

    /**
     *
     */
    public Drafty insertAddMember(int type, String act, String actName, String tgt, String tgtName) {
        if (fmt == null) {
            fmt = new Style[1];
        } else {
            fmt = Arrays.copyOf(fmt, fmt.length + 1);
        ...
        fmt[fmt.length - 1] = new Style(0, 1, ent.length - 1);

        Map<String, Object> data = new HashMap<>();
//        data.put("mime", "text/plain");
        data.put("act", act);
        data.put("actName", actName);
        data.put("tgt", tgt);
        data.put("tgtName", tgtName);
        data.put("type", type);
        ent[ent.length - 1] = new Entity("MEMBER", data);
     ...
     */
    public Drafty insertImageSticker(String fname, String localPath) {
        if (fmt == null) {
            fmt = new Style[1];
        } else {
            fmt = Arrays.copyOf(fmt, fmt.length + 1);
        ...
        Map<String, Object> data = new HashMap<>();
        data.put("mime", "image/*");
        data.put("val", localPath);
        data.put("name", fname);
        ent[ent.length - 1] = new Entity("STICKER", data);
     ...
     * 位置消息
     */
    public Drafty insertLocation(double lat, double lng, String name, String imgUrl) {
        ...
        data.put("lat", lat);
        data.put("lng", lng);
        ent[ent.length - 1] = new Entity("LOC", data);
     ...
     * 实时位置消息
     */
    public Drafty insertRealTimePosition(double lat, double lng) {
        ...
        data.put("lat", lat);
        data.put("lng", lng);
        ent[ent.length - 1] = new Entity("RTP", data);
     ...
     *
     */
    public Drafty insertRecall(String seq) {
        if (fmt == null) {
            fmt = new Style[1];
        } else {
            fmt = Arrays.copyOf(fmt, fmt.length + 1);
        ...
        fmt[fmt.length - 1] = new Style(0, 1, ent.length - 1);
        txt = seq;
        ent[ent.length - 1] = new Entity("RECALL", new HashMap<>());
...
//        }
     ...
     * @param fname  name of the file to suggest to the receiver
     */
    public Drafty insertImage(int at, String mime, String url, int width, int height, String fname, String localPath) {
//        if (txt == null || txt.length() < at + 1 || at < 0) {
//            throw new IndexOutOfBoundsException("Invalid insertion position");
//        }
        if (fmt == null) {
        ...
        data.put("height", height);
        data.put("name", fname);
        data.put("local", localPath);
        ent[ent.length - 1] = new Entity("IMAGE", data);
     ...
     */
    public Drafty insertVoice(String mime, String url, String fname, int length) {
        if (fmt == null) {
            fmt = new Style[1];
        } else {
            fmt = Arrays.copyOf(fmt, fmt.length + 1);
        ...
        ent[ent.length - 1] = new Entity("VC", data);

        return this;
    }

    public Drafty attachFile(String mime, String url, String fname, long size, String cover, String localPath) {
        return attachFile(mime, null, fname, url, size, localPath, cover, SEND_FILE_TYPE_TINODE);
    }


    public Drafty attachFile(String mime, String url, String fname, long size, String cover, String localPath, int type) {
        return attachFile(mime, null, fname, url, size, localPath, cover, type);
    }

    public Drafty attachImage(String mime, String url, String fname, long size, String localPath) {
        return attachImage(mime, fname, url, size, localPath,  SEND_FILE_TYPE_TINODE);
    }
     ...
     * @param fname Optional file name to suggest to the receiver.
     */
    public Drafty attachFile(String mime, byte[] bits, String fname, String cover) {
        return attachFile(mime, bits, fname, null, 0, null, cover, SEND_FILE_TYPE_TINODE);
    }
    ...
    public Drafty attachFileCover(Cover cover) {
//        if (cover != null && cover.refurl != null) {
//            String mime = cover.mime;
//            String fname = cover.fname;
//            String refurl = cover.refurl;
////            String localPath = cover.localPath;
...
//        }
    ...
    public static final int SEND_FILE_TYPE_TINODE = 1;
    public static final int SEND_FILE_TYPE_SEAWEED = 2;
     ...
     */
    protected Drafty attachFile(String mime, byte[] bits, String fname, String refurl, long size, String localPath, String cover, int type) {
        ...
        //接入tinode自带的文件发送，之前val表示文件路径，更新后，val只表示文件内容。
        if (bits != null) {
            data.put("val", bits);
        }
        if (fname != null && !fname.equals("")) {
    ...
    protected Drafty attachImage(String mime, String fname, String refurl, long size, String localPath,int type) {
//        if (bits == null && refurl == null) {
//            throw new IllegalArgumentException("Either file bits or reference URL must not be null.");
//        }
