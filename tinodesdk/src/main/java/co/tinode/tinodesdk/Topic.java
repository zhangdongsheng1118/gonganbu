...
public class Topic<DP, DR, SP, SR> implements LocalData, Comparable<Topic> {
    ...
    protected Date mSubsUpdated = new Date(1414213562373L);
    ...
    protected int state;
    //////////////extend////////////////
    protected int isTop = -1;
    protected String draft;
    protected Date lastTs;
    protected String userName;
    protected int msgBeginId = 0;  // for msg fetch

    //added by pcg
    protected String lastAct;
    protected String lastMsg;

    public int getBeginId() { // for msg fetch
        return msgBeginId;
    }
    ...
    public void setIsTop(int isTop) {
        if(isTop != this.isTop && this.state == Topic.STATE_DEL) {
            setTouched(new Date()); // 这里要妥协一下，服务侧时间和客户侧时间不一致，但是体验会好一些
            setState(Topic.STATE_NORMAL);
        }
        this.isTop = isTop;
    }
    ...
    protected Topic(Tinode tinode, Subscription<SP, SR> sub) {
        ...
        if (mDesc.merge(sub)) {
            if(mDesc.priv != null) {
                PrivateType pt = (PrivateType) mDesc.priv;
                Integer ptTop = pt.getTop();
                if (isTop == -1) {
                    isTop = ptTop == null ? 0 : ptTop;
                } else if (ptTop != isTop) {
                    PrivateType pType = new PrivateType();
                    pType.setTop(isTop);
    ...
    protected void update(Subscription<SP, SR> sub) {
        ...
        if (sub.act != null && !sub.act.equals("")) {
            lastAct = sub.act;
            changed = true;
        }

        if (sub.ct != null && !sub.ct.equals("")) {
            lastMsg = sub.ct;
            changed = true;
        }
    ...
    public int getMsgBeginId() {
        return msgBeginId;
    }

    public void setMsgBeginId(int id) {
        int value = id > mDesc.recv ? id : mDesc.recv;
        if (value > msgBeginId) {
            msgBeginId = value;
        }
    }
    ...
    public int getUnreadCount() {
        int unread = mDesc.seq - mDesc.recv;
     ...
     */
    public long publishPreFile(Drafty content) throws Exception {
        final long id;
        if (mStore != null) {
            id = mStore.msgSendLocal(this, content);
        } else {
            id = -1;
        }
        return id;
     ...
     * @throws NotConnectedException  if there is no connection to server
     */
    public PromisedReply<ServerMessage> publishAfter(long id, Drafty content) throws Exception {

        if (mStore != null) {
            mStore.msgUpdate(id, content);
        }
                ...
                new PromisedReply.FailureListener<ServerMessage>() {
                    ...
                    public PromisedReply<ServerMessage> onFailure(Exception err) throws Exception {
                        if (mStore != null) {
                            ...
                            mStore.msgFailed(Topic.this, msgId, content);
                        }
                        ...
                        public PromisedReply<ServerMessage> onFailure(Exception err) throws Exception {
                            if (mStore != null) {
                               // mStore.msgSyncing(Topic.this, id, false);
                                mStore.msgFailed(Topic.this, id, content);
                            }
    ...
    public <ML extends Iterator<Storage.Message> & Closeable> PromisedReply<ServerMessage> publishPending()
            throws Exception {
        ML list = mStore.getQueuedMessages(this);
        if (list == null) {
            return new PromisedReply<>((ServerMessage) null);
        }
    ...
    public boolean deleteLocal() {
        ...
        this.state = Topic.STATE_DEL;
        mStore.msgDelete(this,-1,0,-1);
        mDesc.recv = mDesc.seq;
        mDesc.read = mDesc.seq;
        setMsgBeginId(mDesc.seq + 1);
        lastAct = null;
        lastMsg = null;

        return mStore.topicUpdateState(this);
    }

    public boolean resumeLocal() {
        setMsgBeginId(mDesc.seq + 1);
        this.state = Topic.STATE_NORMAL;
        return mStore.topicUpdateState(this);
    }
    ...
    protected void addSubToCache(Subscription<SP, SR> sub) {
        ...
        if (sub.pub == null) {
            ...
            User user = mStore.userGet(sub.user);
            ((Subscription<VCard, ?>) sub).pub = (VCard) user.pub;
        }
    }
...
//        try {
//            Log.d(TAG, "Topic Sub is " + new ObjectMapper().writeValueAsString(newsub));
//            Log.d(TAG, "Topic Sub " + newsub.topic + " is " + new ObjectMapper().writeValueAsString(newsub.pub));
...
//        }
    ...
    protected void routePres(MsgServerPres pres) {
        ...
        if (mListener != null) {
            Log.d(TAG, "line1881 onPres: " + pres.what);
    ...
    public static class MetaGetBuilder {
        ...
        public MetaGetBuilder withGetLaterData2(Integer limit) { // for msg fetch
            Storage.Range r = topic.getCachedMessageRange();

            if (r == null) {
                return withGetData(topic.msgBeginId, null, limit);
            }
            return withGetData(r.max + 1 > topic.msgBeginId ? r.max + 1 : topic.msgBeginId, null, limit);
        }
        ...
        public MetaGetBuilder withGetEarlierData2(Integer limit) {  // for msg fetch
            Storage.Range r = topic.getCachedMessageRange();
            if (r == null) {
                return withGetData(topic.msgBeginId, null, limit);
            }
            return withGetData(topic.msgBeginId, r.min > 0 ? r.min : null, limit);
        }
